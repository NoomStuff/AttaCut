...
const preview = preparePreview(source, previewFolder, null, true, { signal: controller.signal });
...
assert.equal(playable.streams.filter((stream) => stream.type === "audio").length, 2);
assert.equal((await preparePreview(source, previewFolder, 1, true, {})).path, ready.path, "Changing tracks reuses the same preview");
assert.equal(playable.streams[0]!.codec, "h264");
console.log("Jobs passed: cancel, cleanup, retry, collision after planning, preserving completed files, and proxy preview.");
